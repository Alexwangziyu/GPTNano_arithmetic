...
def get_results_dir(config):
    ...
    if os.path.exists(results_dir):
        print(
            f"WARNING: results directory {results_dir} already exists, overwriting..."
        )
        id = 1
...
def evaluate_text(config, model, data, ctx):
    ...
    while True:
        ...
        for count in range(batch_size):
            ...
            X.append(
                torch.from_numpy((data[begin_loc:end_loc]).astype(np.int64)).to(device)
            )
            Y.append(
                torch.from_numpy(
                    (data[begin_loc + 1 : end_loc + 1]).astype(np.int64)
                ).to(device)
            )
...
def get_error_metric(
    y, y_hat, metric_type="accuracy", eps=0, list_not_num=[], list_outlier_num=[]
):
...
def evaluate_addition_new(
    ...
    start = (
        ...
        else "FILE:prompt/prompt_addition_pad_test_0.01.txt"
    )
    device = config["device"]
    max_new_tokens = (
        config["max_new_tokens"] if "max_new_tokens" in config.keys() else num_digit + 1
    )
    if algo_reason:
    ...
    for i in x:
        ...
        with torch.no_grad():
            with ctx:
                ...
                if (
                    all(elem in "0123456789" for elem in c_hat2)
                    and c_hat != ""
                    and c_hat2 != ""
                ):
                    c_hat2 = int(c_hat2)
    ...
    accuracy_dictionary = {
        f"carry{i}": (
            carry_dictionary[f"carry{i}_correct"]
            / carry_dictionary[f"carry{i}_total"]
            * 100
            if carry_dictionary[f"carry{i}_total"] != 0
            else np.nan
        )
...
def evaluate_addition_batch(
    ...
    device = config["device"]
    test_batch_size = (
        config["test_batch_size"] if "test_batch_size" in config.keys() else 128
    )
    max_new_tokens = (
        config["max_new_tokens"] if "max_new_tokens" in config.keys() else num_digit + 2
    )
    ...
    if algo_reason:
        max_new_tokens = (
            80 if ("simple" in config["dataset"] or simple) else 320
        )  # TODO:
    elif add_space:
    ...
    if data_type == "text":
        ...
        test_data_str = generate_data_str(
            test_data_list,
            operator=operator,
            format=data_format,
            train=False,
    ...
    if analyze:
        ...
        error_dict = {
            ...
            "mse": [],
            "normalized_mse": [],
            "digit_wise_difference": [],
            "incorrect_digit_count": [],
        }
    ...
    for prompt_length in prompt_dict.keys():
        ...
        for batch_idx in range(
            math.ceil(len(input_tuple_list) / test_batch_size)
        ):  # range(len(input_tuple_list)//test_batch_size+1):
            batch_list.append(
                input_tuple_list[
                ...
                for i, outcome in enumerate(outcome_list):
                    ...
                    if algo_reason:
                        if "</scratch>\n" in c_hat:
                            ...
                            c_hat2 = c_hat2.replace(" ", "")
                        if (
                            "simple" in config["dataset"] or config["simple"]
                        ) and ".\n" in c_hat:
                            c_hat2 = c_hat2.split(".\n")[1]
                            c_hat2 = c_hat2.split("\n")[0]
                    else:  # plain addition
                    ...
                    if analyze:
                        ...
                        for metric_type in metric_types:
                            error, list_not_num, list_outlier_num = get_error_metric(
                                c,
                                c_hat2,
                                metric_type,
                                eps=config["eps"],
                                list_not_num=list_not_num,
                                list_outlier_num=list_outlier_num,
                            )
                            error_dict[f"{metric_type}"].append(error)
                        error, _, _ = get_error_metric(
                            c,
                            c_hat2,
                            "accuracy",
                            eps=0,
    ...
    if analyze:
        ...
        result_dir = (
            config["result_dir"]
            if "result_dir" in config.keys()
            else get_results_dir(config)
        )
...
def evaluate_addition_multidigit(
    config,
    model,
    ctx,
    encode,
    ...
    digit_accuracy_dictionary = {
        f"digit{i}_accuracy": 0 for i in range(1, num_digit + 2)
    }
...
def evaluate_addition_fewshot_batch(
    config,
    model,
    ctx,
    encode,
...
def get_algo_reasoning_str(
    x, y=0, operator="+", train=True, n=4, simple=False, random_A=False, random_C=False
):
    if operator in ["+", "-", "*"]:
        x, y = str(x), str(y)

        len_x, len_y = len(x), len(y)
        list_x, list_y = [int(digit) for digit in str(x)], [
            int(digit) for digit in str(y)
        ]
    ...
    # AR for multiplication
        ...
        raise ValueError(
            f"Operator {operator} for algorithmic reasoning not supported!"
        )
        ...
        if operator in ["+", "-", "*"]:
            ...
            if train:
                ...
                if format == "plain":
                    ...
                    output_str = get_algo_reasoning_str(
                        x1,
                        x2,
                        operator=operator,
                        train=train,
            ...
            if train:
                if format == "algo_reasoning":
                    output_str = get_algo_reasoning_str(
                        x, operator=operator, train=train
                    )
                else:
...
def get_abc(abc: str):
    [a, b] = abc.split("+")
    c = b.split("=")[1]
    if "\n" == c[-1]:  # handle cases where it ends with '\n'
        c = c[:-1]
    b = b.split("=")[0]
    # a,b = int(a), int(b)
    # c = int(a) + int(b)
...
def evaluate_addition(
    config, model, ctx, encode, decode, verbose=False, num_digit=3, analyze=True
):
    print(
        "Warning: evaluate_addition is deprecated. Use evaluate_addition_new instead."
    )
    model.eval()
    start = (
        config["start"]
    ...
    accuracy_dictionary = {
        f"({i},{j})": (
            digit_dictionary[f"({i},{j})_correct"]
            / digit_dictionary[f"({i},{j})_total"]
            * 100
            if digit_dictionary[f"({i},{j})_total"] != 0
            else np.nan
        )
        for i in range(1, num_digit + 1)
        for j in range(1, num_digit + 1)
    }
    accuracy_dictionary.update(
        {
            f"carry{i}": (
                carry_dictionary[f"carry{i}_correct"]
                / carry_dictionary[f"carry{i}_total"]
...
def print_model_output(
    ...
    start="Twinkle twinkle little star",
):
    num_samples = 1
    # encode the beginning of the prompt
    print(
